...
func TestDoT(t *testing.T) {
	...
	data := getMockDataExampleComDNSSECTrue(&acc, monitors.Success, "dns.google.", "8.8.8.8", "853", "tcp-tls")
...
func TestDoUdp(t *testing.T) {
	...
	require.NoError(t, c.Gather(&acc))
	//override the response time
	data := getMockDataExampleComDNSSECTrue(&acc, monitors.Success, "dns.google.", "8.8.8.8", "53", "udp")
	acc.AssertContainsTaggedFields(t, pluginName, data.GetFields(), data.GetTags())
}

func TestDoNonDNSSEC(t *testing.T) {
	// Init plugin
	var acc testutil.Accumulator
	c := DeepmonDNS{
		ResolverProtocol: "udp",
	...
	require.NoError(t, c.Gather(&acc))
	//override the response time
	data := getMockDataGoogleComDNSSECFalse(&acc, monitors.Success, "dns.google.", "8.8.8.8", "53", "udp")
	acc.AssertContainsTaggedFields(t, pluginName, data.GetFields(), data.GetTags())
}
...
func getMockDataExampleComDNSSECTrue(acc *testutil.Accumulator, result monitors.Result, res_name, res_ip, res_port, res_protocol string) (data monitors.MonitorData[*monitors.DNSData]) {
	...
	for _, p := range acc.Metrics {
		...
		for idx := range p.Fields["records"].([]monitors.DNSRecord) {
...
func getMockDataGoogleComDNSSECFalse(acc *testutil.Accumulator, result monitors.Result, res_name, res_ip, res_port, res_protocol string) (data monitors.MonitorData[*monitors.DNSData]) {
	data = monitors.MonitorData[*monitors.DNSData]{
		Domain: "google.com",
		Data: &monitors.DNSData{
			Result:           result,
